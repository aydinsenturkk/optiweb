...
async function processDirectory(inputDir, outputDir, options) {
        ...
        try {
            ...
            if (['.jpg', '.jpeg', '.png'].includes(ext)) {
                ...
                if (options.resize && options.resize.sizes) {
                    const sizeResults = await processMultiSizeImage(inputFile, absoluteOutputDir, baseName, ext, options);
                    ...
                    if (options.verbose) {
                        ...
                        if (sizeResults.totalSaved > 0) {
                            ...
                            spinner.info(`Total space saved: ${formatBytes(sizeResults.totalSaved)} (-%${reduction})`);
                        }
                    }
                } else {
                    // Optimize image file
                    ...
                    if (options.webp) {
                        outputFilePath = path.join(absoluteOutputDir, `${baseName}.webp`);
                    }
    ...
    if (results.skippedImages > 0) {
        spinner.info(`Number of images skipped because a WebP version exists: ${results.skippedImages}`);
    }
 ...
 * @param {Object} options - Optimization options
 */
async function processMultiSizeImage(inputFile, outputDir, baseName, ext, options) {
